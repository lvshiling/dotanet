...
// 返回：新向量
func NewVector3FromStr(str string) Vector3 {

	//
	xyz_str := strings.Split(str, ",")
	if len(xyz_str) < 3 {
		log.Error("NewVector3FromStr error:" + str)
		return Vector3{X: 0, Y: 0, Z: 0}
	}
	x, err1 := strconv.ParseFloat(xyz_str[0], 64)
	y, err2 := strconv.ParseFloat(xyz_str[1], 64)
	z, err3 := strconv.ParseFloat(xyz_str[2], 64)
	if err1 != nil || err2 != nil || err3 != nil {
		log.Error("NewVector3FromStr error:" + str + "   ParseFloat")
		return Vector3{X: 0, Y: 0, Z: 0}
	}
...
func GetDistance3(a Vector3, b Vector3) float64 {
